...
<body>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="POST" class="row g-3" id="form">
                                <?= csrf_field() ?>
                                <input type="hidden" name="id" value="<?= $batch['batch_id'] ?>">
                                <?php
                                $startYear = date('Y');
                                $numberOfSemesters = 5;
                                    ...
                                    <div class="row g-3">
                                        <div class="col-lg-2">
                                            ...
                                            <select name="school_year" class="form-select">
                                                ...
                                                <option value="<?= $semester ?>"
                                                    <?= ($semester===$batch['school_year']) ? 'selected' : '' ?>>
                                                    <?= $semester ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        ...
                                        <div class="col-lg-6">
                                            ...
                                            <input type="text" class="form-control" name="batch"
                                                placeholder="Enter here" value="<?= $batch['batchName'] ?>">
                                            <div id="batch-error" class="error-message text-danger text-sm"></div>
                                        </div>
                                        <div class="col-lg-2">
                                            <label class="form-label">Section</label>
                                            <input type="text" class="form-control" name="section"
                                                placeholder="Enter here" value="<?= $batch['section'] ?>">
                                        ...
                                        <div class="col-lg-8">
                                            ...
                                            <select name="account" class="form-select">
                                                ...
                                                <option value="<?= $row['account_id'] ?>"
                                                    <?= ($row['account_id']===$batch['account_id']) ? 'selected' : '' ?>>
                                                    <?= $row['employee_id'] ?> -
                                                    <?= $row['fullname'] ?></option>
                                                <?php endforeach;?>
                                            </select>
    ...
    $('#form').submit(function(e) {
        ...
        $.ajax({
            url: "<?= site_url('gradebook/batch/update') ?>",
            method: "POST",
            data: data,
                ...
                if (response.success) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href = "<?=site_url('gradebook/batch')?>";
                        }
                    });
                } else {
